...
export const EditService = ({ id }) => {
    ...
    useEffect(() => {
        const loadService = async () => {
            try {
                const service = await fetchCall(`${import.meta.env.VITE_API_URL_BASE}services/${id}`);
                setServiceId(service.id_servicio);
                setServiceName(service.nombre);
                setDescription(service.descripcion);

                // Convertimos el JSON en array [{ name, type }]
                const dynamicFields = Object.entries(service.datos || {}).map(
                    ([name, type]) => ({ name, type })
                );
    ...
    const handleSubmit = async (e) => {
        ...
        await fetchCall(`${import.meta.env.VITE_API_URL_BASE}services/${serviceId}`, "PUT", {}, {
            nombre: serviceName,
            descripcion: description,
            datos: jsonData
        });

        setSuccessMessage('✅ Servicio editado correctamente.'); // Msj éxito
    ...
    return (
        <section className="container d-flex flex-column align-items-center justify-content-center mb-5 pb-4 px-4">
            ...
            >
                <h3 className="fw-bold mb-3">Editar Servicio</h3>
                ...
                <button type="submit" className="btn btn-lg btn-dark w-100 rounded-1">Guardar cambios</button>
